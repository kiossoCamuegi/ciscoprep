...
app.use("/api", async (req, res, next) => {
  try {
    ...
    res.status(500).json({ error: "DB schema init failed" });
  }
});
...
app.post("/api/groups", async (req, res) => {
  ...
  const conn = await pool.getConnection();
  try {
    const [[maxRow]] = await conn.query("SELECT COALESCE(MAX(position), 0) AS maxPos FROM `groups`");
...
app.patch("/api/groups/:id", async (req, res) => {
  ...
  const conn = await pool.getConnection();
  try {
    const [u] = await conn.query(`UPDATE \`groups\` SET ${fields.join(", ")} WHERE id = :id`, params);
